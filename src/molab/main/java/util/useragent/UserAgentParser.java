package molab.main.java.util.useragent;
...
/**
 * A convenience class for parsing the user agent string that is presented by
 * web browsers. This work is based on the great work that is documented at the
 * texsoft website:
 * 
...
public class UserAgentParser {
	...
	public UserAgentParser(String userAgentString) {
		...
		if (parsedBrowsers.size() > 0) {
			...
			throw new UserAgentParseException(
					"Unable to parse user agent string: " + userAgentString);
		}
	...
	private void processBrowserDetails() {
		...
		browserOperatingSystem = extractOperatingSystem(parsedBrowsers.get(0)
				.getBrowserComments());
	 ...
	 * 
	 * @return a string array with browser name in element 0 and browser version
	 *         in element 1. Null can be present in either or both.
	 */
	private String[] extractBrowserNameAndVersion() {
		...
		for (UserAgentDetails nextBrowser : parsedBrowsers) {
			for (String nextKnown : knownBrowsers) {
				if (nextBrowser.getBrowserName().toLowerCase().startsWith(
						nextKnown)) {
	 ...
	 * 
	 * @param comments
	 *            the comment string afer the browser version
	...
	private String extractOperatingSystem(String comments) {
		...
		String[] knownOS = new String[] { "win", "linux", "mac", "freebsd",
				"netbsd", "openbsd", "sunos", "amiga", "beos", "irix", "os/2",
				"warp", "iphone" };
